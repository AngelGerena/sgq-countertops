...
/* Installed work grouped by material. Photo counts are uneven on purpose —
   the page leads with guidance rather than galleries, so a thin section
   reads as a short section, not a broken one. */
const GRANITE: Shot[] = [
...
export default function Stone() {
  ...
  /* Photos are supporting evidence here, not the promise. A one-photo
     section is presented as one photo rather than dressed up as a gallery. */
  const gallery = (list: Shot[], items: ReturnType<typeof set>) => (
    <div className={'stone-shots' + (list.length === 1 ? ' single' : '')}>
      {list.map((s, i) => (
        <figure className={'cab-tile reveal' + (s.feature && list.length > 1 ? ' feature' : '')} key={s.slug}>
  ...
  const shotLabel = (n: number) =>
    n === 1
      ? t('stone.shots.one', 'From a recent install', 'De una instalación reciente')
      : L(`${n} recent installs`, `${n} instalaciones recientes`);
    ...
    <div>
      ...
      <section className="blog-hero">
        <div className="band-in">
          ...
          <p className="band-lede reveal">
            {t('stone.lede',
              'Twenty-five years of installs, condensed into the handful of things that actually decide it. Read this before the showroom and you will walk in knowing what you are looking at.',
              'Veinticinco años de instalaciones, resumidos en las pocas cosas que realmente deciden. Lea esto antes de ir a la sala de exhibición y sabrá exactamente qué está viendo.')}
          </p>
        </div>
      </section>

      {/* ---------- COMPARISON FIRST — this is the reason to be on the page ---------- */}
      <section className="band dark" id="compare">
        <div className="band-in">
          ...
          <p className="band-lede reveal">
            {t('stone.compare.lede',
              'Four questions separate these three materials. Everything after that is personal taste.',
              'Cuatro preguntas separan estos tres materiales. Todo lo demás es gusto personal.')}
          </p>
            ...
            <table className="stone-table">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="hl">{t('stone.never', 'Never', 'Nunca')}</td>
                  <td>{t('stone.yes.year', 'Yearly', 'Anual')}</td>
                </tr>
                <tr>
                  <td>{t('stone.row.heat', 'Hot pan straight down', 'Olla caliente directa')}</td>
                  <td className="hl">{t('stone.yes', 'Yes', 'Sí')}</td>
                  <td>{t('stone.trivet', 'Use a trivet', 'Use salvamanteles')}</td>
                  <td className="hl">{t('stone.yes', 'Yes', 'Sí')}</td>
                </tr>
                <tr>
                  <td>{t('stone.row.unique', 'Every slab one of a kind', 'Cada losa única')}</td>
                  <td className="hl">{t('stone.yes', 'Yes', 'Sí')}</td>
          ...
          <div className="stone-picks">
            <div className="pick reveal">
              <span className="pick-k">{t('stone.pick.1.k', 'If you cook hard', 'Si cocina fuerte')}</span>
              <p>{t('stone.pick.1.v',
                'Granite or quartzite. Pans come off the burner and go straight down, and nothing marks.',
                'Granito o cuarcita. Las ollas salen del fuego y van directo a la encimera, sin marcar nada.')}</p>
            </div>
            <div className="pick reveal">
              <span className="pick-k">{t('stone.pick.2.k', 'If you want it low-effort', 'Si quiere poco mantenimiento')}</span>
              <p>{t('stone.pick.2.v',
                'Quartz. No sealing, ever. Wine, oil and lemon do not touch it.',
                'Cuarzo. Sin sellado, nunca. El vino, el aceite y el limón no le hacen nada.')}</p>
            </div>
            <div className="pick reveal">
              <span className="pick-k">{t('stone.pick.3.k', 'If you want what nobody else has', 'Si quiere algo que nadie más tiene')}</span>
              <p>{t('stone.pick.3.v',
                'Granite or quartzite, picked in person. You approve the exact slab before we cut it.',
                'Granito o cuarcita, elegido en persona. Usted aprueba la losa exacta antes de cortarla.')}</p>
            </div>
            <div className="pick reveal">
              <span className="pick-k">{t('stone.pick.4.k', 'If it has to match a sample', 'Si tiene que coincidir con una muestra')}</span>
              <p>{t('stone.pick.4.v',
                'Quartz. Engineered, so what you pick is exactly what arrives.',
                'Cuarzo. Es diseñado, así que lo que elige es exactamente lo que llega.')}</p>
            </div>
          </div>
      ...
      <section className="band" id="granite">
        <div className="band-in">
          ...
          <span className="shot-label reveal">{shotLabel(GRANITE.length)}</span>
      ...
      <section className="band dark" id="quartz">
        <div className="band-in">
          ...
          <span className="shot-label reveal">{shotLabel(QUARTZ.length)}</span>
      ...
      <section className="band" id="quartzite">
        <div className="band-in">
          ...
          <span className="shot-label reveal">{shotLabel(QUARTZITE.length)}</span>
